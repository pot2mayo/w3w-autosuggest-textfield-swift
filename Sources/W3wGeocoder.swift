...
public typealias W3wGeocodeResponseHandler  = ((_ result: ([String: Any], Data)?, _ error: W3wError?) -> Void)
public typealias W3wResponsePlace           = ((_ result: (W3wPlaceJson?, W3wPlaceGeojson?)?, _ error: W3wError?) -> Void)
...
public class W3wGeocoder {
    ...
    public func convertToCoordinates(words: String, format: Format = .json, completion: @escaping W3wResponsePlace) {
        ...
        self.performRequest(path: "/convert-to-coordinates", params: params) { (result, error) in
            ...
            switch format {
            case .json:
                if let place = result?.0 {
                    completion((W3wPlaceJson(result: place), nil), nil)
                } else {
            ...
            case .geojson:
                if let data = result?.1 {
                    ...
                    do {
                        let object = try JSONDecoder().decode(W3wPlaceGeojson.self, from: data)
                        completion((nil, object), nil)
                    } catch {
                        let w3wError = W3wError(code: "ParseFail", message: error.localizedDescription)
                        completion(nil, w3wError)
                    }
                } else {
                    completion(nil, error)
                }
    ...
    public func convertTo3wa(coordinates: CLLocationCoordinate2D, language: String = "en", format: Format = .json, completion: @escaping W3wResponsePlace) {
        ...
        self.performRequest(path: "/convert-to-3wa", params: params) { (result, error) in
            if let place = result?.0 {
                completion((W3wPlaceJson(result: place), nil), error)
            } else {
                completion(nil, error)
            }
        }
...
/// W3wPlaceGeojson type definition
public struct Geometry: Codable {
    public let coordinates: [Double] // [-0.195521,51.520847] lng/lat
    public let type: String // "Point"
}

public struct Properties: Codable {
    public let country: String // "GB"
    public let nearestPlace: String // "Bayswater, London"
    public let words: String // "filled.count.soap"
    public let language: String // "en"
    public let map: String // "https://w3w.co/filled.count.soap"
}

public struct Feature: Codable {
    public let bbox: [Double] // [-0.195543,51.520833,-0.195499,51.52086]
    public let geometry: Geometry
    public let type: String
    public let properties: Properties
}

public struct W3wPlaceGeojson: Codable {
    public let features: [Feature]
    public let type: String // "FeatureCollection"
}
